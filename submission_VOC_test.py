...
class evaluate(object):
    ...
    def __init__(self, test_file, test_flag, save_path):
        ...
        self.network = VOC_model.Model().new_network_1(self.inputs)

        self.anchor_boxes = VOC_model.Model().anchor_boxes
        ...
        with tf.Session() as sess:
            ...
            try:
                ...
                for i in truth_boxes:
                    ...
                    xmax, ymax = min(boxes[i][3]*width/self.image_size, float(width)), min(boxes[i][2]*height/self.image_size, float(height))
                    for k in self.VOC_LABELS.keys():
                        if self.VOC_LABELS[k] == cls[i]:
                            self.result_str[self.VOC_LABELS[k]] += '%s %f %f %f %f %f\n' % (img_path[:-4], scores[i], xmin+1., ymin+1., xmax, ymax)
                            # files[self.VOC_LABELS[k]].write('%s %f %f %f %f %f\n' % (img_path[:-4], scores[i], xmin, ymin, xmax-xmin, ymax-ymin))
                            break
